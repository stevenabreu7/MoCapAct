...
model_config = config_flags.DEFINE_config_file("model", f"{config_file}:mlp_time_index", "Model architecture")
...
def pad_collate(batch):
    """Create batches from sequences of unequal episode lengths.
    Padding with zeros.
    """
    (xx, yy, zz) = zip(*batch)

    x_lens = [len(x[list(x.keys())[0]]) for x in xx]
    y_lens = [len(y) for y in yy]
    z_lens = [len(z) for z in zz]

    assert y_lens == z_lens == x_lens, 'x, y, z lengths must be equal'

    xx_pad = {
        obs_key: torch.nn.utils.rnn.pad_sequence(
            [torch.Tensor(x[obs_key]) for x in xx], batch_first=True
        ) for obs_key in xx[0].keys()
    }
    yy_pad = torch.nn.utils.rnn.pad_sequence(
        [torch.Tensor(y) for y in yy], batch_first=True
    )
...
def main(_):
    ...
    is_rnn = 'RnnPolicy' in FLAGS.model.constructor
    ...
    ds_cls = dataset.ExpertDatasetRNN if is_rnn else dataset.ExpertDataset
    ...
    feat_extr_kwargs = dict(observable_keys=FLAGS.model.config.observables,
                            obs_rms=obs_rms)
    policy = model_constructor(
        ...
        features_extractor_kwargs=feat_extr_kwargs,
        **FLAGS.model.config
    )

    collate_fn = pad_collate if is_rnn else None
    ...
    val_loader = None
    if FLAGS.val_dataset_paths is not None:
        if FLAGS.validation_freq is not None:
    ...
    for eval_mode in FLAGS.eval_mode:  # Policy evaluation callbacks
        ...
        prefix = eval_mode
        if os.getenv("LOCAL_RANK", "0") == "0":
            pth = Path(osp.join(output_dir, 'eval', prefix))
            pth.mkdir(parents=True, exist_ok=True)
        if is_train_dataset:
            snippets = train_dataset.clip_snippets_flat
        elif eval_mode.startswith("val"):
            snippets = val_dataset.clip_snippets_flat
        else:
            snippets = FLAGS.extra_clips
    ...
    trainer = pl.Trainer(
        ...
        val_check_interval=val_check_int,
        deterministic=True,
        benchmark=True,
        callbacks=train_callbacks,
        logger=[csv_logger, tb_logger]
    )
    trainer.fit(policy, train_loader, val_dataloaders=val_loader,
                ckpt_path=FLAGS.load_path)
