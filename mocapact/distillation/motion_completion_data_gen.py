...
flags.DEFINE_integer("n_eval_episodes", 20, "Number of episodes to numerically evaluate policy")
flags.DEFINE_integer("n_workers", 10, "Number of parallel workers")
flags.DEFINE_string("eval_save_path", "./", "If desired, the path to save the evaluation results")

# Sequence output Parameters
flags.DEFINE_integer("min_length",100,"Minimum length of the saved sequence")
flags.DEFINE_integer("num_seqs",100,"number of saved sequence")
...
def main(_):
    ...
    @torch.no_grad()
    def policy_fn(time_step, env, policy):
        ...
        t += 1
        return action, env, policy,obs
        
    cnt = 0
    while cnt < FLAGS.num_seqs:
            
        obs_writer = open(f".observations_{cnt}.json", "w")
        obs_writer.write('{"observations": [')
        act_writer = open(f".actions_{cnt}.json", "w")
        act_writer.write('{"actions": [')
        
        with torch.no_grad():
            time_step = env._env.reset()
            while not time_step.last():
                action, env, policy, obs = policy_fn(time_step, env, policy)    
                time_step = env._env.step(action)
                
                obs_, _ = policy.obs_to_tensor(obs)
                features = policy.features_extractor(obs_)
                dict_obs = {f"timestep {t}": features.tolist()}
                json.dump(dict_obs,obs_writer)
                obs_writer.write(",")
                
                list_action = action.tolist()
                dict_action = {f"timestep {t}": list_action}
                json.dump(dict_action, act_writer)
                act_writer.write(",")
